...
import org.testng.annotations.BeforeTest;
	...
	public class GPSOT_903  extends Project {

			@BeforeTest

			public void setData() {
				testCaseName = "GPSOT_903";
				testCaseDesc = "Initial Project Setup: RPS Account Fields in Onboarding Org (Generic Business Firm Record Type) in Salesforce";
				category = "GPSOTest";
				author = "Suraj";
			...
			public void GPSOT_903_Test(Map<String, String> details) throws InterruptedException, IOException {
				...
				*/	//click(locateElement("xpath", "//button[@class='slds-button slds-show']"));	
				...
				try {
					...
					driver.findElement(By.xpath("//input[@name='Account-search-input']")).sendKeys("Nationwide");
					driver.findElement(By.xpath("//input[@name='Account-search-input']")).sendKeys(Keys.ENTER);
					click(locateElement("xpath", "//a[text()='Nationwide']"));
					...
					locateElement("xpath", "//label[text()='Firm Roles']").isDisplayed();
				...
				//Competitor firm Information Section

				try {
					WebElement webElement1 = driver.findElement(By.xpath("//span[text()='Competitor Firm Information']"));
					((JavascriptExecutor)driver).executeScript("arguments[0].scrollIntoView();", webElement1);									
					//locateElement("xpath", "//span[text()='Competitor Firm Information']").isDisplayed();
					
					locateElement("xpath", "//label[text()='Transfer Fees']").isDisplayed();
					locateElement("xpath", "//label[text()='Deconversion Package']").isDisplayed();
					locateElement("xpath", "//label[text()='Signature Guarantee']").isDisplayed();
					locateElement("xpath", "//label[text()='Other Conversion Details']").isDisplayed();				
					locateElement("xpath", "//label[text()='$ Amount']").isDisplayed();
					locateElement("xpath", "//label[text()='Blackout Period']").isDisplayed();
					locateElement("xpath", "//label[text()='Notice Requirement']").isDisplayed();				
					

					reportStep("Competitor firm information section fields are isplayed", "pass", takeSnap(getTitle()));
				

			} catch (Exception e) {
				reportStep("Failed to display Competitor firm information section fields", "fail", takeSnap(getTitle()));
				e.printStackTrace();
			}
