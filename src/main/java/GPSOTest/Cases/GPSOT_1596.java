package GPSOTest;
	...
	public class GPSOT_1596 extends Project {

			@BeforeTest

			public void setData() {
				testCaseName = "GPSOT_1596";
				testCaseDesc = "SF - Create Public Groups for Case Object";
				category = "GPSOTest";
				author = "Suraj";
			...
			public void GPSOT_1596_Test(Map<String, String> details) throws InterruptedException, IOException {
				...
				try {
					...
					try {
						...
						click(locateElement("xpath", "//a[text()='US Case Read']"));
						Thread.sleep(2000);
						driver.switchTo().frame(driver.findElement(By.xpath("//iframe[@title='Group: US Case Read ~ Salesforce - Unlimited Edition']")));
						locateElement("xpath", "//td[text()='Label']//following-sibling::td[text()='US Case Read']").isDisplayed();//label name
						
						locateElement("xpath", "//td[text()='Group Name']//following-sibling::td[text()='US_Case_Read']").isDisplayed();//Group Name
						...
						Thread.sleep(3000);	
						driver.switchTo().defaultContent();
						reportStep("Case Read Page displayed", "pass", takeSnap(getTitle()));
						

					} catch (Exception e) {
						reportStep("Failed to display Case Read Page", "fail", takeSnap(getTitle()));
						e.printStackTrace();
					}
					try {
						click(locateElement("xpath", "//a[text()='Public Groups']"));
						...
						click(locateElement("xpath", "//a[text()='US Case Read/Write']"));
						Thread.sleep(2000);
						driver.switchTo().frame(driver.findElement(By.xpath("//iframe[@title='Group: US Case Read/Write ~ Salesforce - Unlimited Edition']")));
						locateElement("xpath", "//td[text()='Label']//following-sibling::td[text()='US Case Read/Write']").isDisplayed();//label name
						
						locateElement("xpath", "//td[text()='Group Name']//following-sibling::td[text()='US_Case_Read_Write']").isDisplayed();//Group Name
						locateElement("xpath", "//td[text()='Grant Access Using Hierarchies']//following-sibling::td//img[@title='Checked']").isEnabled();//Grant access using hierarchies
						...
						Thread.sleep(3000);	
						driver.switchTo().defaultContent();
						reportStep("Case Read/Write Page displayed", "pass", takeSnap(getTitle()));
						

					} catch (Exception e) {
						reportStep("Failed to display Case Read/Write Page", "fail", takeSnap(getTitle()));
						e.printStackTrace();
					}
					try {
