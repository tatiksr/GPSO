...
public class GPSOT_735 extends Project {

	@BeforeTest

	public void setData() {
		testCaseName = "GPSOT_735";
		testCaseDesc = "Additional fields to Project Object - Plan Fees, Project Name Logic";
		category = "GPSOTest";
		author = "Suraj";
	...
	public void GPSOT_735_Test(Map<String, String> details) throws InterruptedException, IOException {
		...
		// Step 4 : Verify the display of the 'Plan Fees' section
				try {
					waitPageLoading();
					waitPageLoading();
					driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
					JavascriptExecutor je = (JavascriptExecutor) driver;
					WebElement planfees = driver.findElement(By.xpath("//span[text()='Plan Fees']"));
					je.executeScript("arguments[0].scrollIntoView(true);",planfees);
				
				if (locateElement("xpath", "//span[text()='Plan Fees']").isDisplayed()) {
					reportStep("Plan Fees section is displayed successfully", "pass", takeSnap(getTitle()));
				} else {
					reportStep("Plan Fees section is not displayed successfully", "fail", takeSnap(getTitle()));
				}
			}
				catch (Exception e) {
					reportStep("Failed to display Plan Fees", "fail", takeSnap(getTitle()));
					e.printStackTrace();
				}
